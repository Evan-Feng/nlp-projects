class Vocab(object):
    ...
    def frequency_cutoff(self, cutoff_frequency=5):
        self._idx2w = [w for w in self._idx2w if self._counts[w] >= cutoff_frequency]
        self._w2idx = {w: i for i, w in enumerate(self._idx2w)}
        self._counts = {w: c for w, c in self._counts.items() if w in self._w2idx}
